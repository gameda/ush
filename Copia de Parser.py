...
def addVariableLocal(idVar, tipo, largo, array, scope):
    ...
    if existVariableGlobal(idVar) or existMetodos(idVar):
        ...
        tipo = diccionario_metodos.get(scope).getVariable(varLoc).getTipo()
        diccionario_metodos.get(scope).getVariable(varLoc).setDireccion(vars_dir[tipo])
...
def p_asignacion(p):
    ...
    elif(diccionario_metodos.get(scopeActual).has_key(p[1])):
...
def error_semantic(p): 
    print ("No es posible realizar la operacion en la linea: %s " % p)
    sys.exit()
